...
function build_3D_PDE(N, nx, dx, dt, Tmax, Tstart; dense::Bool = true, implicit = false)
    ...
    Q  = 10. * Matrix(LinearAlgebra.I, ns, ns) * dx^2
    Qf = 10. * Matrix(LinearAlgebra.I, ns, ns)./dt * dx^2
    R  = 1.0 * Matrix(LinearAlgebra.I, nu, nu)./10 * dx^2
    ...
    if dense
        if implicit
            lqdm = DenseLQDynamicModel(s0, A, B, Q, R, N; Qf = Qf, sl = sl, su = su, ul = ul, uu = uu, implicit=implicit)
        else
            lqdm = DenseLQDynamicModel(s0, A, B, Q, R, N; Qf = Qf, sl = sl, su = su, ul = ul, uu = uu)
        end
    else
        lqdm = SparseLQDynamicModel(s0, sparse(A), sparse(B), sparse(Q), sparse(R), N; Qf = sparse(Qf), sl = sl, su = su, ul = ul, uu = uu)
    end
